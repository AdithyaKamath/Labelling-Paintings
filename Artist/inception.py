...
if(flag == 0):
    ...
    for i in range(x_train.shape[0]):
        ...
        img = image.img_to_array(image.load_img(image_name, target_size = (299,299)))
        images[i] = img
        if i % 2000 == 0:
            print(i)
    #sd = np.std(images)
    ...
    if save:
        ...
        np.save(store_path + "features_irv2_train.npy", features_test)
else:
    print("Loading features from files")
    features_train = np.load(store_path + "features_irv2_train.npy")
    features_test = np.load(store_path + "features_irv2_train.npy")
